...
// LGE_CHANGE_S [myeonggyu.son@lge.com] [gelato] set-up manual mode on setup wizard [START]
//minsu.jeong 20110606 add test script mode
	if(((req_ptr->sub_cmd_code == TEST_MODE_MANUAL_MODE_TEST) && (req_ptr->test_mode_req.test_manual_mode == MANUAL_TEST_ON))
		|| ((req_ptr->sub_cmd_code == TEST_MODE_TEST_SCRIPT_MODE) && (req_ptr->test_mode_req.test_mode_test_scr_mode == TEST_SCRIPT_ITEM_SET)))
	{
		if(diagpdev!=NULL)
		{
			printk(KERN_INFO "%s : manual mode on\n",__func__);
			update_diagcmd_state(diagpdev, "MANUALMODE", 1);
		}
	}
// LGE_CHANGE_S [myeonggyu.son@lge.com] [gelato] set-up manual mode on setup wizard [START]
...
#endif
// LGE_CHANGE_E [myeonggyu.son@lge.com] add testmode cmd [END]
...
#if 1//def CONFIG_LGE_CHARGING_MODE_INFO
...
extern int lge_retry_erase_block(int ebnum);
extern int lge_retry_write_block(int ebnum, unsigned char *buf, size_t size);
		...
		DIAG_TEST_MODE_F_rsp_type	*pRsp)
{
	...
	switch(pReq->factory_reset)
	{
		case FACTORY_RESET_CHECK :
			...
			mtd_op_result = lge_retry_erase_block(mtd_factory_blk);
#else
			...
			{
#ifdef LGE_RETRY_MTD_DIRECT_ACCESS
				printk(KERN_ERR "[Testmode]lge_retry_erase_block, error num = %d \n", mtd_op_result);
#else
			...
			{
#ifdef LGE_RETRY_MTD_DIRECT_ACCESS
				mtd_op_result = lge_retry_write_block(mtd_factory_blk, pbuf, BUF_PAGE_SIZE);
#else
				...
				{
#ifdef LGE_RETRY_MTD_DIRECT_ACCESS
					printk(KERN_ERR "[Testmode]lge_retry_write_block, error num = %d \n", mtd_op_result);
#else
...
int factory_reset_check(void)
{
			...
			test_mode_factory_reset_status == 6 /* 6 : value to indicate to disable usb debug setting for ui factory reset*/)
	{
		...
		{
#ifdef LGE_RETRY_MTD_DIRECT_ACCESS
			printk(KERN_ERR "factory_reset_check : lge_retry_erase_block, error num = %d \n", mtd_op_result);
#else
		...
		else
		{
			...
			{
#ifdef LGE_RETRY_MTD_DIRECT_ACCESS
				printk(KERN_ERR "factory_reset_check : lge_retry_write_block, error num = %d \n", mtd_op_result);
#else
...
void* LGF_TestModeDBIntegrityCheck(test_mode_req_type* pReq ,DIAG_TEST_MODE_F_rsp_type	   *pRsp)
{
	...
	printk("<choi> LGF_TestModeDBIntegrityCheck start\n");
	...
	if (diagpdev != NULL){
			...
			switch(pReq->db_check)
			{
				case DB_INTEGRITY_CHECK:
					printk("<choi> LGF_TestModeDBIntegrityCheck DB_INTEGRITY_CHECK before while\n");
					while ( !db_integrity_ready )
						msleep(10);
					printk("<choi> LGF_TestModeDBIntegrityCheck DB_INTEGRITY_CHECK after while\n");
...

					{
						...
						printk(KERN_INFO "<choi>%s\n", integrity_ret.ret);
						printk(KERN_INFO "<choi>%ld\n", crc_val);
				...
				case FPRI_CRC_CHECK:
					printk("<choi> LGF_TestModeDBIntegrityCheck FPRI_CRC_CHECK before while\n");
					while ( !fpri_crc_ready )
						msleep(10);
					printk("<choi> LGF_TestModeDBIntegrityCheck FPRI_CRC_CHECK after while\n");
					fpri_crc_ready = 0;
				...
				case FILE_CRC_CHECK:
				{
					printk("<choi> LGF_TestModeDBIntegrityCheck FILE_CRC_CHECK before while\n");
					while ( !file_crc_ready )
						msleep(10);
					printk("<choi> LGF_TestModeDBIntegrityCheck FILE_CRC_CHECK after while\n");
					file_crc_ready = 0;
