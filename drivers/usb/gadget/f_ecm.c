...
static void ecm_do_notify(struct f_ecm *ecm)
{
	...
	case ECM_NOTIFY_SPEED:
		...
		//20110325 yongman.kwon@lge.com [LS855] for ecm driver notify [START]
#if 0	//def CONFIG_USB_SUPPORT_LGE_ANDROID_ECM_FIX
		req->length = 16;
#else
...
#else
#ifdef CONFIG_LGE_USB_GADGET_DRIVER
//		if (product_id == 0x61FC)		// activate when PID 0x6310 USB driver released.
		if (product_id == 0x6310)
		{
			ecm_string_defs[1].id = 5;
			pr_info("%s: iMACAddress = %d\n", __func__, status);
			ecm_desc.iMACAddress = 5;
