...
/* LGE_CHANGES_S [moses.son@lge.com] 2011-04-03 when powering off, do not reset USB */
extern bool is_kernel_power_off;
/* LGE_CHANGES_E [moses.son@lge.com] 2011-04-03 */
...
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB //ulpi_read from SBA 2030_07
...
/* CONFIG_MACH_LGE_BRYCE hyunjong.do@lge.com 10.09.21
  add detection of charger type  */
#if 0	//def CONFIG_MACH_LGE_GELATO
	udc_cable = get_msm_cable_type();
	/*2010.12.23 jaeho.cho to detect LT cable as usb cable */
	if (udc_cable==LG_WALL_CHARGER_CABLE)
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	/* LGE_CHANGE_S [bk.shin@lge.com] 2011-03-15, Factory Cable current config */
	/* orgi : if (maxpower > 0) */	
	if (maxpower >= 0)
		otg_set_power(ui->xceiv, maxpower);
	/* LGE_CHANGE_E [bk.shin@lge.com] 2011-03-15, Factory Cable current config */
...
int usb_ept_queue_xfer(struct msm_endpoint *ept, struct usb_request *_req)
{
	...
	if (ui->usb_state == USB_STATE_SUSPENDED) {
		if (!atomic_read(&ui->remote_wakeup)) {
			...
			/* LGE_CHANGES_S [moses.son@lge.com] 2011-05-11 */
//			ui->flags |= USB_FLAG_RESET;
//			usb_reset_adb();
			/* LGE_CHANGES_E [moses.son@lge.com] 2011-05-11 */
			return -EAGAIN;
		}
...
static int user_cable = 0;

static void usb_reset(struct usb_info *ui)
{
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
	unsigned long flags;
//	extern int lg_manual_test_mode;
	int lg_manual_test_mode;
	...
	writel(ui->dma, USB_ENDPOINTLISTADDR);
...
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
#if 1
		/* [yk.kim@lge.com] 2011-01-13, prevent muic cable detect delay */
		udc_cable = get_msm_cable_type();
		if(udc_cable == LG_NORMAL_USB_CABLE)
			user_cable = 1;
#endif	
#if 1	//def CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
		lg_manual_test_mode = msm_get_manual_test_mode();

		if(udc_cable==LG_UNKNOWN_CABLE && lg_manual_test_mode && !user_cable)
		{
		...
		if(udc_cable==LG_FACTORY_CABLE_56K_TYPE)
		{
			unsigned tmp = 0; 
		...
		if(udc_cable==LG_FACTORY_CABLE_56K_TYPE || udc_cable==LG_FACTORY_CABLE_130K_TYPE || udc_cable==LG_FACTORY_CABLE_910K_TYPE)
		{
...
#if 1	// moses.son@lge.com need to check
void usb_reset_adb(void)
{
	unsigned long flags;
	struct usb_info *ui = the_usb_info;
