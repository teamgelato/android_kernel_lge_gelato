...
#if defined(CONFIG_FB_MSM_MDDI_AUO_HVGA)
#define REGFLAG_END_OF_TABLE      0xFFFF   // END OF REGISTERS MARKER
...
extern void auo_display_table(struct display_table *table, unsigned int count);
#endif
...
static void mdp_dma2_update_lcd(struct msm_fb_data_type *mfd)
{
	...
	/* LGE_CHANGE
	  * Change to apply DMA_DITHER_EN code according to Thunder Project.
	  * 2011-04-08, hoseok.kim@lge.com
	  */
...
/* LGE_CHANGE [dojip.kim@lge.com] 2010-04-23, [LS670] fixed the pixel format */
...
#else /* original */
		MDP_OUTP(MDP_BASE + 0x00094,
		#if defined(CONFIG_MACH_MSM7X27_THUNERC)
			/* Don't apply 6013 patch only when using Hitachi HVGA module. 2010-07-28. minjong.gong@lge.com */
			#if defined (CONFIG_FB_MSM_MDDI_HITACHI_HVGA)
				(MDDI_VDO_PACKET_DESC << 16) | mddi_vdo_packet_reg);
			#else
	...
	hitachi_display_table(mddi_hitachi_position_table, 
		sizeof(mddi_hitachi_position_table) / sizeof(struct hitachi_display_table));
#endif
	...
	auo_display_table(mddi_auo_position_table, 
		sizeof(mddi_auo_position_table) / sizeof(struct display_table));
...
#endif
{
	...
	if ((mfd) && (!mfd->dma->busy) && (mfd->panel_power_on)) {
		...
		/* LGE_CHANGE_S, 2011-05-24. hoseok.kim@lge.com */
		/* wait until DMA finishes the current job */
		if(wait_for_completion_killable(&mfd->dma->comp) < 0)
			goto out;
		/* LGE_CHANGE_E, 2011-05-24. hoseok.kim@lge.com */
