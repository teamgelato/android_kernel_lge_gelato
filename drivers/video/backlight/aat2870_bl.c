...
#define HW_LCD_TUNNING_SCENARIO_3
#endif
...
#define LCD_LED_MAX 29 /* 28.06 mA */
#define LCD_LED_MIN 26  /* 25.16 mA */
#elif defined(HW_LCD_TUNNING_SCENARIO_2)
#define LCD_LED_MAX 21 /* 20.32 mA */
#define LCD_LED_MIN 13 /* 12.58 mA */
#elif defined(HW_LCD_TUNNING_SCENARIO_3)
#define LCD_LED_MAX 21 /* 20.40 mA */
#define LCD_LED_MIN 0 /*  0.48mA */
#else
...
#elif defined(HW_LCD_TUNNING_SCENARIO_3)
#define DEFAULT_BRIGHTNESS 13 /* 12.72 mA */
#else
#define DEFAULT_BRIGHTNESS 13
#endif
#endif // CONFIG_MACH_MSM7X27_GELATO_DOP
...
#if defined(CONFIG_LGE_PCB_REV_B) && !defined(CONFIG_MACH_MSM7X27_GELATO_DOP)
#define AAT2862BL_REG_PSB	0x08  /* Register address for PSB */
#endif
...
#define DEBUG 1 // hoseok.kim@lge.com
#if DEBUG
...
#if defined(CONFIG_LGE_PCB_REV_B) && !defined(CONFIG_MACH_MSM7X27_GELATO_DOP)
		drvdata->reg_addrs.psb= AAT2862BL_REG_PSB;
#endif
	}
...
/* LGE_CHANGE.
  ...
  */
	...
	/* LGE_CHANGE.
	* aat28xx PWM enable when system booting.
	* 2011-02-22, hoseok.kim@lge.com
	*/
#if defined(CONFIG_LGE_PCB_REV_B) && !defined(CONFIG_MACH_MSM7X27_GELATO_DOP)
	aat28xx_write(drvdata->client, drvdata->reg_addrs.psb, 0x03);
#endif
}
...
static void aat28xx_sleep(struct aat28xx_driver_data *drvdata)
{
	...
	switch (drvdata->mode) {
		case NORMAL_MODE:
			...
			*/
#if defined(CONFIG_LGE_PCB_REV_B) && !defined(CONFIG_MACH_MSM7X27_GELATO_DOP)
			dprintk("PWM enable & PSB disable");
//~			aat28xx_write(drvdata->client, drvdata->reg_addrs.psb, 0x02);
//pwn enable psb enable
			aat28xx_write(drvdata->client, drvdata->reg_addrs.psb, 0x03);
#endif
			break;
...
#if defined(CONFIG_LGE_PCB_REV_B) && !defined(CONFIG_MACH_MSM7X27_GELATO_DOP)
				aat28xx_intensity = 0x9F;							/* MEQS(7)=1, Disable Fade(6)=0, LCD_ON(5)=0*/
				aat28xx_write(drvdata->client, drvdata->reg_addrs.bl_m, aat28xx_intensity);
#endif
				aat28xx_intensity = (~(drvdata->intensity)& 0x1F);	/* Invert BL control bits and Clear upper 3bits */
//				aat28xx_intensity |= 0xA0;							/* MEQS(7)=1, Disable Fade(6)=0, LCD_ON(5)=1*/
...
/* LGE_CHANGE_S [hoseok.kim@lge.com] 2011-03-11, backlight on/off */
...
/* LGE_CHANGE_E [hoseok.kim@lge.com] 2011-03-11, backlight on/off */
...
/* LGE_CHANGE [hoseok.kim@lge.com] 2011-03-11, backlight on/off */
