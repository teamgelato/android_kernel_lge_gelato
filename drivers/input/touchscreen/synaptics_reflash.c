...
#include "synaptics_img_ls685.h"
#else
#include "synaptics_tm1827.h" // lgit Touch Window FW
#include "synaptics_tm1936.h" // suntel Touch Window FW
#endif
...
void SynaReadFirmwareInfo()
{
  ...
  printk(KERN_INFO "synaptics_ts_fw_angent : SynaReadFirmwareInfo -SynafirmwareImgSize=0x%x \n",SynafirmwareImgSize);
}
...
void SynaReadConfigInfo()
{
  ...
  printk(KERN_INFO "synaptics_ts_fw_angent : SynaReadConfigInfo -SynaconfigImgSize=0x%x \n",SynaconfigImgSize);
}
...
void SynaReadFirmwareHeader()
{
  unsigned long firmwareImgFileCheckSum = 0;
  unsigned long checkSumCode = 0;
  
  if(name_of_product == SYNAPTICS_LGIT) // checksum fail
  {
	  ...
	  { 
	    SynaPrintMsg("\nError: SynaFirmwareImage invalid checksum.firmwareImgFileCheckSum=%d,checkSumCode=%d\n");
	    SynaExit();		
	  }
  ...
  else /* suntel SynaFirmware_1936 */
  {
  	  SynafileSize = sizeof(SynaFirmware_1936) - 1;
     
	  checkSumCode            = ExtractLongFromHeader(&(SynaFirmware_1936[0]));
	  SynabootloadImgID       = (unsigned int)SynaFirmware_1936[4] + (unsigned int)SynaFirmware_1936[5]*0x100;
	  SynafirmwareImgVersion  = SynaFirmware_1936[7];	
	  SynafirmwareImgSize     = ExtractLongFromHeader(&(SynaFirmware_1936[8]));
	  SynaconfigImgSize       = ExtractLongFromHeader(&(SynaFirmware_1936[12]));		

	  SynaCalculateChecksum((unsigned short*)(&SynaFirmware_1936[4]), (unsigned short)((SynafileSize - 4) >> 1),
	                        (unsigned long *)(&firmwareImgFileCheckSum));
	  ...
	  SynafirmwareImgData = (unsigned char *)((&SynaFirmware_1936[0])+0x100);
	  SynaconfigImgData   = (unsigned char *)((&SynaFirmware_1936[0])+0x100+SynafirmwareImgSize);
...
{ 
  ...
  if(name_of_product == SYNAPTICS_LGIT)
  {
    if((fw_revision >= SynaFirmware[0x1F]) && (is_need_forced_update == 0)||(is_need_forced_noupdate==1))
    {
      printk(KERN_INFO "synaptics_ts_fw_angent : F/W version is up-to-date\n");
    	return -1;
    }
	printk(KERN_INFO "synaptics_ts_fw_angent : lgit F/W Upgrade!! - from 0x%x to 0x%x\n",fw_revision,SynaFirmware[0x1F]);
 
  }
#ifndef CONFIG_MACH_MSM7X27_GELATO_DOP  
  else
  {
  	if((fw_revision >= SynaFirmware_1936[0x1F]) && (is_need_forced_update == 0)||(is_need_forced_noupdate==1))
    {
      printk(KERN_INFO "synaptics_ts_fw_angent : F/W version is up-to-date\n");
    	return -1;
    }
	printk(KERN_INFO "synaptics_ts_fw_angent : suntel F/W Upgrade!! - from 0x%x to 0x%x\n",fw_revision,SynaFirmware_1936[0x1F]);
  }
#endif   
  is_fw_reflash = 1;
  ...
  printk(KERN_ERR "synaptics_ts_fw_angent : SynaInitialize end!!\n");
